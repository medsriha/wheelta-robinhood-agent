...
version: 6
status: draft (ADR-0006, ADR-0007, ADR-0010, ADR-0011, ADR-0012, ADR-0018). Replaces v5 (adds
  position notes); never run live yet.
...
placeholders:
  ...
  position_book       durable entry facts, thesis, events known at entry, roll lineage, and
                      notes from earlier runs per active position (ADR-0018)
...
7. Each position book entry carries `notes`: your own rationale, thesis, and open questions
   about that position from earlier runs, oldest first, until it closes. `notes_omitted`
   counts older notes not shown. Use them for continuity: what you were watching, why you
   held, which question was open. They are not evidence. Re-verify with current tools, cite
   only this run's references, and change course when current evidence says so.
