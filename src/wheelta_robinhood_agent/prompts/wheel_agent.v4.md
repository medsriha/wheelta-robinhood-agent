...
version: 4
status: archived draft, never run. Superseded by v5 (ADR-0012); not selected for new runs.
...
2. Use only the sources and precedence allowed by `data_quality`. Material contradictions
   exclude the dependent trade; cite both observations in the decision or rejection.
...
Return one JSON object matching AgentDecisionOutput v4. Return only your selections,
...
- rejected_candidates entries contain target_ref, rationale, evidence_refs. unresolved_questions
  entries contain nullable target_ref, question, evidence_refs. These are qualitative judgments
  and research needs; code generates missing/stale/contradictory-data gaps from evidence.
